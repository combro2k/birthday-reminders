...
impl ExportService {
    ...
    pub async fn export_birthdays_for_user(&self, user_id: &UserId) -> anyhow::Result<String> {
        ...
        let rows: Vec<BirthdayExportRow> = birthdays.into_iter().map(BirthdayExportRow::from_birthday).collect();
    ...
    pub async fn export_all_birthdays(&self) -> anyhow::Result<String> {
        ...
        let rows: Vec<BirthdayExportRow> = all_birthdays.into_iter().map(BirthdayExportRow::from_birthday).collect();
        self.write_csv(&rows, "birthdays.csv")
    }
    ...
    pub async fn export_api_tokens(&self, user_id: &UserId, db: &DatabasePool) -> anyhow::Result<String> {
    ...
    pub async fn export_notifications(&self, user_id: &UserId, db: &DatabasePool) -> anyhow::Result<String> {
    ...
    pub async fn export_reminder_settings(&self, user_id: &UserId, db: &DatabasePool) -> anyhow::Result<String> {
        let rows = self.fetch_reminder_settings(user_id, db).await?;
        self.write_csv(&rows, "reminders.csv")
    }

    /// Fetch all API tokens for a user
    async fn fetch_api_tokens(&self, user_id: &UserId, db: &DatabasePool) -> anyhow::Result<Vec<ApiTokenExportRow>> {
        match db {
            DatabasePool::Postgres(pool) => {
                ...
                Ok(rows
                    .into_iter()
                    .map(|(id, user_id, token_hash, name, created_at, last_used_at)| {
                        ApiTokenExportRow {
                            id,
                            user_id,
                            token_hash,
    ...
    async fn fetch_reminder_settings(&self, user_id: &UserId, db: &DatabasePool) -> anyhow::Result<Vec<ReminderSettingExportRow>> {
        match db {
            DatabasePool::Postgres(pool) => {
    ...
    pub fn write_csv_files(&self, data: Vec<(&str, String)>, output_dir: &str) -> anyhow::Result<()> {
